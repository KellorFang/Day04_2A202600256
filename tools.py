...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        for item in expenses.split(","):
            ...
            parts = item.split(":")
            if len(parts) != 2:
                return f"Lỗi định dạng: '{item}'. Đúng định dạng: 'tên_khoản:số_tiền'."
            name = parts[0].strip().replace("_", " ")
            amount = int(parts[1].strip())
            expense_dict[name] = amount
    except ValueError as e:
        return f"Lỗi khi đọc số tiền: {e}. Vui lòng nhập số nguyên (VD: 890000)."
    ...
    for name, amount in expense_dict.items():
        ...
        lines.append(f"    - {name.capitalize()}: {amt_fmt}đ")

    lines.append("    ---")
    lines.append(f"    Tổng chi:   {f'{total_expenses:,}'.replace(',', '.')}đ")
    lines.append(f"    Ngân sách:  {f'{total_budget:,}'.replace(',', '.')}đ")

    if remaining < 0:
        over = f"{abs(remaining):,}".replace(",", ".")
        lines.append(f"    Còn lại:    -{over}đ")
        lines.append(f"\n⚠️  Vượt ngân sách {over}đ! Cần điều chỉnh.")
    else:
        rem_fmt = f"{remaining:,}".replace(",", ".")
        lines.append(f"    Còn lại:    {rem_fmt}đ")
